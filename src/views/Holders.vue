<template>
    <div>
            ...
            <section class="section-shaped my-0">
                ...
                <div class="container shape-container d-flex">
                    <div class="col px-0">
                        <div class="row">
                            <div class="col-lg-6">
                                <h1 class="display-3 text-white">Angry Polar Bears
                                    <span>우리의 홀더들</span>
                                </h1>
                            </div>
                        </div>
        ...
        <section class="section-xl pt-lg-0 mt--200">
            <div class="px-4">
                <div class="container">
                  <div class="row justify-content-center">
                    <img class="rounded-circle col-sm-3"
                         src="https://storage.googleapis.com/bucket-angrypolarbears/bears/0.png" alt=""
                         v-on:click.prevent="getHolders"/>
                  </div>
                </div>
            </div>
        ...
        <section class="section pt-lg-0 mt--200" v-if="ListFlag">
            <div class="section-sm container row-grid" v-for="Hold in ['1','2','3']" :key="Hold">
                <div class="row justify-content-center">
                    <div class="col-lg-12">
                      <h3 class="text-Default font-weight-bold">지갑주소 <small>/ 3개</small></h3>
                        <div class="row row-grid">
                            <div class="col-lg-3 row-grid section-sm" v-for="Hold_list in ['1', '2', '3','4','5', '6', '7','8']" :key="Hold_list">
                                <card class="border-0" hover shadow>

                                    <img v-lazy="'https://storage.googleapis.com/bucket-angrypolarbears/bears/0.png'"
                                       class="rounded-circle img-center img-fluid shadow shadow-lg--hover"
                                       style="width: 200px;">
                                    <div class="mt-3">
                                        <badge type="primary" rounded>#0000</badge>
                                        <badge type="primary" rounded>365일</badge>
                                    </div>
                                    <hr>
                                </card>
                            </div>
      ...
      <modal :show.sync="modalFlag"
        ...
        <h6 slot="header" class="modal-title" id="modal-title-notification">안 내</h6>

        <div class="py-3 text-center">
          <i class="ni ni-bell-55 ni-3x"></i>
          <h4 class="heading mt-4">You should read this!</h4>
          <p>홀더들이 너무 많아서 조회 하는 데에 20초 가량 걸립니다!</p>
        </div>
        ...
        <template>
          ...
          <base-button type="white"
                       class="ml-auto"
                       @click="modalFlag = false">Ok, Got it</base-button>
          </div>
        </template>
      </modal>
<!--
  ...
  methods: {
    ...
    getHolders: function () {
      ...
      const queryString = new URLSearchParams({
        ...
        "cursor" : this.reqData.cursor
      });

      const axiosReq = axios.create({
        baseURL: "https://th-api.klaytnapi.com/v2/contract/nft/0xc984335f3b02382ec7393f9e091d19b84064589b/token" ,
        headers:{
          "Authorization": "Basic S0FTS1NaVDhHVkc5QjlQSlVDNjM1TElKOmNEMnVGQkpJTVg0LXhDSWNIdFM1NGgyMGk5cS1SazQ5WFJhd1JUWWI=",
          "x-chain-id": "8217",
      ...
      console.log(axiosReq.defaults.baseURL);


      axiosReq.get(axiosReq.defaults.baseURL, config)
          .then((res) => {
        if((res.status !== 200)||(res.data.code !== 0)) {
          //알람
        } else {
          this.HolderListRes.itmes.add(res.data);

          if((res.data.cursor !== undefined) || (res.data.cursor !== "")) {
            this.reqData.cursor = res.data.cursor;
            this.getHolders();
          }
        }
      });
